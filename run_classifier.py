...
def file_based_convert_examples_to_features(
  ...
  for (ex_index, example) in enumerate(examples):
    ...
    # features["label_ids"] = create_int_feature(literal_eval(feature.label_id))


    label_converted = literal_eval(feature.label_id)

    if (isinstance(label_converted, list)):
        features["label_ids"] = create_int_feature(label_converted)
    else:
        features["label_ids"] = create_int_feature([label_converted])
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    # total_loss = tf.argmax(total_loss)
    ...
    d = tf.data.Dataset.from_tensor_slices({
        ...
        "segment_ids":
            tf.constant(
                ...
                dtype=tf.int32),
        "label_ids":
            tf.constant(all_label_ids, shape=[num_examples, num_examples], dtype=tf.int32),
    })
